...
Candidate selection narrows by ZIP, then exact street identity, and accepts a result only when the surviving candidates describe one place (spread ≤ 0.5 mi) **and sit in one county**. Several distinct real locations → `tiger_ambiguous`, no pin. No fuzzy street matching. Wrong pin remains worse than no pin.

**ZIP is a hard constraint (fixed 2026-08).** When the member supplies a ZIP, a candidate must belong to it; there is **no statewide fallback** to a same-named street in another town → `tiger_zip_mismatch`, no pin. Ordinal street words are normalized to digits in the shared layer (`Sixth St` → `6TH STREET`, `Twenty First St` → `21ST STREET`), so spelled-out and numeric forms share one street identity; the stored public `street_key`/`street_core` values were regenerated in place from the same normalizer. Root cause of the earlier `150 Sixth St, Ely, NV 89301` misplacement was these two defects together.
...
**Failure taxonomy (each a real outcome, all distinct from a missing provider).** `tiger_not_a_street_address`, `tiger_out_of_state`, `tiger_no_match`, `tiger_out_of_range`, `tiger_ambiguous`, `tiger_zip_mismatch`, `tiger_unavailable`. The "Automatic member address lookup is not configured" message is **removed** — `useMemberAccess.ts` now shows the real outcome (ambiguous / out of range / Nevada-only / needs street number / temporarily unavailable), keeping service-unavailable and highway/manual messages distinct.
...
- **Tests:** `src/test/resourceGeocodeConsolidation.test.ts` (39 tests). `resourceGeocodeCache.test.ts` and `resourceCacheCorrectness.test.ts` were updated to the Census provider contract.

### Resource geocoding validation + legacy dry-run closure (Phase 6f.1 — Phase 2D.1)

Corrective pass on Phase 6f. No production legacy coordinates were migrated.

- **Physical street-identity validation.** `supabase/functions/_shared/streetIdentity.ts` deterministically parses and compares the source street against the Census `matchedAddress` (house number, alias-expanded street name, street type, edge directionals; suite/unit noise stripped). `validateCensusMatch` now rejects an unrelated street inside the same ZIP (`street_identity_unverified`, `house_number_match: false`, `street_name_match: false`) and reports `street_verdict` (`match | mismatch | insufficient_evidence`). No fuzzy, name-based or probabilistic matching — insufficient evidence rejects rather than guesses.
- **One protection contract.** `supabase/functions/_shared/resourceEligibility.ts` (`evaluateResourceEligibility`) is now used by **both** `geocode-bulk` and `geocode-address`. `force` may re-resolve a record that already has coordinates but can never bypass soft-delete, `mappable = false`, `active_status = false`, or manual/locked coordinate authority. The previous single-record drift (a `force` path that overwrote locked rows) is closed.
- **Superseded legacy provenance preserved.** `resourceCachePorts.upsert` merges metadata via `buildSupersededMetadata`: replacing a `google`/`nominatim` cache row appends a `source_metadata.superseded_resolution` entry (previous provider, coordinates, confidence, precision, `superseded_at`, replacement provider), keeps earlier history, and caps it at `MAX_SUPERSESSION_HISTORY = 5`.
- **Record-level history.** Both functions write audit details on legacy transition: `previous_provider`, `previous_coordinate_source`, `previous_latitude/longitude`, `new_provider`, `new_coordinate_source`, `new_latitude/longitude`, `distance_meters`, `legacy_provider_superseded`.
- **Combined read-only dry-run.** `mode: 'dry_run_revalidation'` now covers **all 9 tables in one report** with no `table` required. Addresses are grouped by the *same* canonical identity the cache keys on (`buildResourceAddress` → `canonicalizeAddress`), so Census is called once per unique address deduplicated **across tables**, and every record sharing that address receives its own comparison row and its own `distance_meters`. Counters are distinct and truthful (`records_with_legacy_provenance`, `unique_canonical_legacy_addresses`, `records_without_deterministic_address_identity`, `census_attempted/resolved/unresolved`, `validation_rejected`, `protected_records`, `records_compared`). Distances are reported as factual distribution (min/median/p75/p90/p95/max) and buckets (`0-25m`, `25-100m`, `100-500m`, `500m-1km`, `1-5km`, `>5km`) with no invented correctness threshold. `limit`/`offset` slice only the external call volume per invocation (`next_offset`); grouping and inventory always span every table. The path performs **zero** writes: no cache upsert, no `touch`, no counter increments, no coordinate/provenance/review-status change. Admin UI: Geocode Health → "Legacy provenance dry-run" (Admin/SysOp only, JSON download).
- **Legacy inventory (from DB, at implementation time):** 353 records still carry `geocode_provider = google` — `rural_services` 172, `verified_services` 72, `verified_bh` 56, `facilities` 53. No `nominatim` rows remain. Staging tables have null provenance. Replacement of these coordinates remains a separate, deliberate, not-yet-performed operation.
- **Tests:** `src/test/resourceGeocodeValidation.test.ts` (61 tests). `src/test/geocodeForceSemantics.test.ts` (16 tests). Full suite: 399 tests green.
- **Real force semantics (final closure).** `geocode-address` now calls `evaluateResourceEligibility(record, contract, { force: !!force })` — the earlier unconditional `force: true, allowExistingCoordinates: true` is removed. Caller intent is explicit: `triggerGeocodeAddress(table, id, { force: true })` is used ONLY where `street_address` actually changed (`editFacilityRecord`, `editProviderStaging`); new-record/import/promotion enrichment (`insertStagingProviders`, `promoteStagingFacility`) stays non-force, so coordinates that arrived with the import are skipped with `already_has_coordinates` and never replaced by Census. A non-force skip performs zero Census calls and mutates nothing (no `coordinate_source = failed`, no provider clear, no confidence or `last_geocoded_at` change). Manual/locked coordinates stay protected under force.
- **Provider copy.** Geocode Review names Census as the active external provider; Google/Nominatim are described as retired legacy provenance that remains reviewable (the review filter still includes those `coordinate_source` values).
