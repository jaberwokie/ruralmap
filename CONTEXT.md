...
- **Cache reuse does not depend on Google.** `geocode-address` evaluates `GOOGLE_GEOCODING_API_KEY` only when building the external provider chain, after the internal authority has been consulted. Cache hit + missing/unavailable Google → success with `cache_hit: true`, `external_calls: 0`. Cache miss + missing credential → `google_credentials_missing` (503), no record stamp, no cache mutation, no invented coordinate.
...
**Legacy browser resource-geocoding surface (inventory only — next phase):** `src/utils/serviceGeocode.ts` still calls public Nominatim search and reverse directly from the browser (`geocodeOne`, `geocodeMany`, `fetchNominatim`, `reverseGeocode`, `spotCheckCoordinate`) and the unauthenticated `census-geocode` proxy (`fetchCensusGeocode`), bypassing the `resource_address` cache. Reachable through `mappingPipelineStore` bulk operations (`geocodeStagingServicesBulk`, `geocodeStagingBhBulk`, `geocodeStagingFacilitiesBulk`, `geocodeStagingRuralServicesBulk`, `geocodeFacilitiesBulk`, `geocodeRuralServicesBulk`) and `providerStagingStore`, surfaced in AdminMappingServices, AdminMappingBehavioralHealth, AdminMappingFacilitiesStaging, AdminMappingRuralServicesStaging. Up to 4 external requests per record (2 Nominatim search + Census + reverse spot check). Not modified in Phase 2C.1.
