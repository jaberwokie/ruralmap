...
import {
  listStagingRuralServices,
  editStagingRuralServiceRecord,
  rejectStagingRuralService,
...
export default function AdminMappingRuralServicesStaging() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const data = await listStagingRuralServices();
    setRows(data);
    setLoading(false);
  }, []);
  ...
  const handleExport = () => {
    ...
    exportCsv(
      ...
      'staging_rural_services_export.csv',
    );
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <PipelineWorkspace
        title="Rural Services Staging Pipeline"
        purpose="Upload, validate, geocode, and promote rural service records. Records must be promoted before appearing on the live map."
        status="active"
        schemaSections={[
          {
            heading: 'Identity',
        ...
        validationRules={['Name is required', 'Category is required']}
        template={{
          filename: 'rural_services_staging_template.csv',
          headers: ['name', 'category', 'city', 'county', 'street_address', 'state', 'zip', 'phone', 'website'],
          exampleRow: {
            name: 'Example Rural Service',
        ...
        onPromote={async (id) => {
          try {
            ...
            toast.success('Rural service promoted to live map');
            await refresh();
          } catch (e: any) {
