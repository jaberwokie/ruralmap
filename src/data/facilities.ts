...
export const defaultFacilities: Facility[] = [
  ...
  { id: "h2", name: "Banner Churchill Community Hospital", type: "hospital", classification: "cah", city: "Fallon", county: "Churchill", address: "801 E Williams Ave", lat: 39.4762, lng: -118.7662, notes: "Regional hospital", accessType: "Rural",
    inpatient: { inpatient_services_offered: true, inpatient_service_types: ['medical inpatient (general acute)'], inpatient_verification_status: 'verified_via_directory', inpatient_accepting_admissions: 'yes', inpatient_medicaid_status: 'participating', inpatient_referral_pathway: 'ED_required', inpatient_bed_availability_model: 'daily_call_required', inpatient_capacity_notes: 'Rural hospital; capacity variable; confirm via ED', inpatient_population_focus: 'adult', inpatient_access_notes: 'Inpatient available; psychiatric cases may transfer out', inpatient_transfer_dependency: 'moderate' } },
  ...
  { id: "t14", name: "Dr. Ronald Pak, PsyD LLC", type: "clinic", classification: "clinic_provider", city: "Pahrump", county: "Nye", address: "311 S Frontage Rd, Ste 106, Pahrump, NV 89048", lat: 36.22144, lng: -115.99446, service: "BH", volume: 1086, tier: "tier1", dataConfidence: "Likely Accurate" },
...
// ── Derived county-level flags ──

/** Returns true if at least one hospital entity exists in the given county */
export const countyHasHospital = (county: string): boolean =>
  defaultFacilities.some(f => f.type === 'hospital' && f.county === county);

/** Returns list of counties (from facility data) that have zero hospital entities */
export const getCountiesWithoutHospitals = (): string[] => {
  const allCounties = new Set(defaultFacilities.map(f => f.county));
  return Array.from(allCounties).filter(c => !countyHasHospital(c));
};

/** Count of counties where no provider has verified psychiatric status */
export const getCountiesWithZeroVerifiedPsychiatricProviders = (): string[] => {
  const VERIFIED: string[] = ['directly_verified', 'verified_via_directory'];
  const allCounties = new Set(defaultFacilities.map(f => f.county));
  return Array.from(allCounties).filter(county => {
    const countyProviders = defaultFacilities.filter(f => f.county === county && f.type !== 'hospital');
    return !countyProviders.some(f =>
      f.psychiatric?.psychiatric_verification_status != null &&
      VERIFIED.includes(f.psychiatric.psychiatric_verification_status)
    );
  });
};
